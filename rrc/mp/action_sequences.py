...
class ScriptedActions(object):
    ...
    def add_move(self, tip_pos, unit_length, min_height=0.01):
        ...
        self.tip_positions_list += complete_keypoints(
            current_tip_pos, tip_pos, unit_length=unit_length
        )

    def add_grasp(self, coef=0.9):
        target_tip_positions = self.grasp.T_cube_to_base(self.grasp.cube_tip_pos * coef)
    ...
    def add_release(self, coef=2.0, min_height=0.01):
        ...
        if np.any(target_tip_positions[:, 2] < min_height):
            target_tip_positions[:, 2] = np.maximum(
                target_tip_positions[:, 2], min_height
            )
    ...
    def add_move_to_center(self, coef=0.6):
        ...
        self.add_move(self.grasp.T_cube_to_base(self.grasp.cube_tip_pos * coef), 0.004)
    ...
    def add_heuristic_pregrasp(self, pregrasp_tip_pos):
        if self.get_last_tippos()[:, 2].min() < CUBOID_SIZE[0] / 2:
            print(
                "Warning: adding heuristic pregrasp even though robot_tip postiion is low"
            )
    ...
    def add_pitch_rotation(self, height, rotate_axis, rotate_angle, coef=0.6):
        # lift cube up
        self.grasp.update(self.grasp.pos + np.array([0, 0, height]), self.grasp.quat)
        target_tip_positions = self.grasp.T_cube_to_base(self.grasp.cube_tip_pos * coef)
        ...
        print(
            f"add_pitch_rotation: rotate_axis {rotate_axis}\trotate_angle {rotate_angle}"
        )
    ...
    def add_yaw_rotation(self, goal_quat, step_angle=np.pi / 3, coef=0.9):
        ...
        ori = (
            R.from_euler("Z", angle_clipped) * R.from_quat(self.grasp.quat)
        ).as_quat()
