...
from rrc.pybullet_planning.utils import CLIENT, DEFAULT_EXTENTS, DEFAULT_HEIGHT, DEFAULT_RADIUS, \
...
from rrc.pybullet_planning.interfaces.env_manager.pose_transformation import unit_pose, multiply, unit_point, unit_quat
...
def create_plane(normal=[0, 0, 1], mass=STATIC_MASS, color=BLACK):
    from rrc.pybullet_planning.interfaces.robots.body import set_texture, set_color
...
def vertices_from_data(data):
    from rrc.pybullet_planning.interfaces.env_manager.pose_transformation import apply_affine
    from rrc.pybullet_planning.interfaces.env_manager.shape_creation import get_data_type, get_data_extents, get_data_radius, get_data_height, \
    ...
    from rrc.pybullet_planning.interfaces.geometry.bounding_box import AABB, get_aabb_vertices
...
def collision_shape_from_data(data, body, link, client=None):
    from rrc.pybullet_planning.interfaces.env_manager.pose_transformation import multiply
    from rrc.pybullet_planning.interfaces.robots.dynamics import get_joint_inertial_pose
...
def clone_collision_shape(body, link, client=None):
    from rrc.pybullet_planning.interfaces.env_manager.shape_creation import get_collision_data
    client = get_client(client)
