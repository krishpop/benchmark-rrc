...
def main(args):
    ...
    print(
        "Mean elapsed time\t{:.2f}".format(sum(wallclock_times) / len(wallclock_times))
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=10,
        type=int,
        help="number of episodes to record a video",
    )
    parser.add_argument("--seed", default=0, type=int, help="random seed")
    parser.add_argument(
        "--policy", default="mpfc", choices=["fc", "mpfc"], help="which policy to run"
    )
    args = parser.parse_args()

    print(
        "For faster recording, run `git apply faster_recording_patch.diff`. This temporary changes episode length and window size."
    )
