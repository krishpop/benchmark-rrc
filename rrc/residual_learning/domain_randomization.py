...
import numpy as np
import gym
from rrc.mp.const import CUBOID_MASS
...
class ParameterDict(object):
    ...
    def test(self, name, low=False, high=False):
        ...
        if low and high:
            raise ValueError(
                "Can't test parameter. You must set exactly one of 'low' or 'high' to True."
            )
        if low:
            ...
            raise ValueError(
                "Can't test parameter. You must set 'low' or 'high' to True."
            )
        return params
...
class TriFingerRandomizer(object):
    def __init__(
        ...
        dynamics_params.append(
            Parameter(
                "cube_mass",
                (2 - cube_mass_scale) * CUBOID_MASS,
                cube_mass_scale * CUBOID_MASS,
            )
        )
        ...
        self.robot_params = ParameterDict(
            *[
                Parameter(
                    "robot_position", 9 * [-position_scale], 9 * [position_scale]
                ),
                Parameter(
                    "robot_velocity", 9 * [-velocity_scale], 9 * [velocity_scale]
                ),
        ...
        self.action_params = ParameterDict(
            *[
                Parameter(
                    "action_position", 9 * [-action_position], 9 * [action_position]
                ),
