...
from rrc.residual_sac import ResidualSAC
import dl
from dl import nest
import torch
import numpy as np
...
class TrainingUtil(object):
    ...
    def _add_action_to_obs(self, obs, ac):
        ...
        if ac["position"] is None:
            ...
            obs["action"]["position"] = np.zeros_like(
                self.env.action_space["position"].low
            )
    ...
    def get_observation(self, obs, base_action):
        ...
        obs = nest.map_structure(
            norm, nest.zip_structure(obs, self.ob_norm["mean"], self.ob_norm["std"])
        )
