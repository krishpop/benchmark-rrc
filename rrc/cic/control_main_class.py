...
class TriFingerController:
    ...
    def position_control_fingers(self, model, data, q, v, location, threshold):
        ...
        for i in range(3):
            dist = np.sqrt(
                np.sum(np.square(data.oMi[JOINT_ID[i]].translation - location[:, i]))
            )

            q_des = self.kinematics.inverse_kinematics(
                model,
                data,
                copy.deepcopy(q),
                copy.deepcopy(location[:, i]),
                JOINT_ID[i],
            )
            ac[i * 3 : (i + 1) * 3] = copy.deepcopy(
                self.pos_ctrl(
                    q[i * 3 : (i + 1) * 3],
                    v[i * 3 : (i + 1) * 3],
                    q_des[i * 3 : (i + 1) * 3],
    ...
    def cartesian_position_control(self, model, data, q, v, location, threshold):
        ...
        for i in range(3):
            ...
            q_er = jac @ er
            ac[i * 3 : (i + 1) * 3] = (
                q_er[JOINTS[i][0] : JOINTS[i][1]]
                - np.array([0.1, 0.1, 0.1])
                * v[self.end_effectors_id[i] - 4 : self.end_effectors_id[i] - 1]
            )
...
class TriFingerKin:
    ...
    def get_tip_pos_pinnochio(self, q):
        ...
        return [
            self.data.oMi[4].translation,
            self.data.oMi[8].translation,
            self.data.oMi[12].translation,
        ]
    ...
    def inverse_kinematics_3_fingers(self, q, fing1_des, fing2_des, fing3_des):
        ...
        des_joints[:3] = copy.deepcopy(
            self.inverse_kinematics(copy.deepcopy(q), fing1_des, 4)
        )[:3]
        des_joints[3:6] = copy.deepcopy(
            self.inverse_kinematics(copy.deepcopy(q), fing2_des, 8)
        )[3:6]
        des_joints[6:9] = copy.deepcopy(
            self.inverse_kinematics(copy.deepcopy(q), fing3_des, 12)
        )[6:9]
    ...
    def imp_ctrl(self, q, force_dir, joint_id, vel, pos, v, K_xy, K_z):
        ...
        J_var_frame = pin.getJointJacobianTimeVariation(
            self.model, self.data, joint_id, pin.ReferenceFrame.LOCAL_WORLD_ALIGNED
        )[:3, :]
    ...
    def imp_ctrl_3_fingers(
        self, q, force_dir, joint_id, vel, pos, v, K_xy, K_z, center_arr
    ):
        ac = np.zeros(9)
        for i in range(3):
            if not (center_arr[i]):
    ...
    def comp_combined_force(self, q, des_force, des_torque, in_touch):
        ...
        for i in range(3):
            if in_touch[i] == 1:
                ...
                idx_low = idx - 4
                J_NEW = pin.getJointJacobian(
                    self.model, self.data, idx, pin.ReferenceFrame.LOCAL_WORLD_ALIGNED
                )[:3, :]
                J_trans = J_NEW.T
        ...
        return (add_torque_force1 + add_torque_force2 + add_torque_force3), np.zeros(
            (9)
        )

    def comp_combined_torque(
        self, q, des_force, des_torque, in_touch, center, p1, p2, p3
    ):
        ...
        # input ("NET FORCE")
        return np.zeros((9)), (
            add_torque_force1 + add_torque_force2 + add_torque_force3
        )
    ...
    def add_additional_force_3_fingers(
        self, q, force_factor, edge_dir, center, correct_torque=False
    ):
        in_touch = [int(not (center[0])), int(not (center[1])), int(not (center[2]))]
        if not (center[0]):
            add_torque_force1, zolo = self.comp_combined_force(
                copy.deepcopy(q), -force_factor * edge_dir[0], [0, 0, 0], [1, 0, 0]
            )
        else:
            add_torque_force1 = np.zeros((9))
        if not (center[1]):
            add_torque_force2, zolo = self.comp_combined_force(
                copy.deepcopy(q), -force_factor * edge_dir[1], [0, 0, 0], [0, 1, 0]
            )
        else:
            add_torque_force2 = np.zeros((9))
        if not (center[2]):
            add_torque_force3, zolo = self.comp_combined_force(
                copy.deepcopy(q), -force_factor * edge_dir[2], [0, 0, 0], [0, 0, 1]
            )
        else:
        ...
        net_force = self.comp_effective_force(
            copy.deepcopy(q), add_torque_through_normal, in_touch
        )
        add_torque_froce, add_torque_torque = self.comp_combined_force(
            copy.deepcopy(q), -net_force, [0, 0, 0], in_touch
        )
        return add_torque_through_normal + add_torque_froce

    def add_additional_force_3_fingers_center_gain(
        self,
        q,
        force_factor,
        force_factor_center,
    ...
    def trafo_q(self, q):
        ...
        assert len(q) == 9, "q should have 9 elements, has {}".format(len(q))
        q_new = np.zeros(12)
        q_new[0:3] = q[0:3]
        q_new[3] = 0.0
        q_new[4:7] = q[3:6]
