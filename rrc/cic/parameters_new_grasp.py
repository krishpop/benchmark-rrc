...
class CubeLvl1Params:
    def __init__(self, env):
        ...
        # PARAMS FOR THE GRASP FLOOR PRIMITIVE:
        self.grasp_xy_rotate_floor = self.grasp_xy_floor = -0.002748661208897829 - 0.04
        self.grasp_h_rotate_floor = self.grasp_h_floor = 0.0
        self.gain_xy_pre_floor = self.gain_xy_pre_rotate_floor = 294.8115234375
        self.gain_z_pre_rotate_floor = self.gain_z_pre_floor = 249.4430389404297
        self.init_dur_rotate_floor = self.init_dur_floor = 100
        if env.simulation:
            self.gain_xy_final_floor = (
                self.gain_xy_final_rotate_floor
            ) = 693.0245697498322
            self.gain_z_final_floor = self.gain_z_final_rotate_floor = 744.9281394481659
            self.pos_gain_impedance_floor = self.pos_gain_impedance_rotate_floor = 0.015
        else:
            self.gain_xy_final_rotate_floor = 294.8115234375
            self.gain_z_final_rotate_floor = 249.4430389404297
            self.pos_gain_impedance_rotate_floor = 0.032040052115917206
        self.force_factor_floor = self.force_factor_rotate_floor = 0.613079309463501
    ...
    def __init__(self, env):
        ...
        if env.simulation:
            ...
            self.grasp_xy_lift = (
                ...
                - 0.04
                + 0.0034238076210021985
                + 0.019999999999999997
            )  # -0.02229524075984955
            self.grasp_h_lift = (
                0.0 + 0.02 - 0.02838870149105787
            )  # -0.012001985907554625
        else:
        ...
        if env.simulation:
            ...
            self.gain_z_lift_lift = (
                744.9281394481659  # 762.2084140777588 #722.6458370685577
            )
            self.pos_gain_impedance_lift_lift = (
                0.01481801524758339  # 0.015682869404554364 #0.06
            )
            self.force_factor_lift = (
                0.32288771867752075  # 0.7260927557945251 #0.7495793342590333
            )
        else:
...
class CubeLvl4Params:
    def __init__(self, env):
        ...
        self.approach_duration = [
            ...
            750,
            900,
        ]  # [250, 500, 750, 850, 1100, 1250]#[500, 1000, 1500, 2000]
        self.approach_clip_pos = [0.03, 0.03, 0.03, 0.03, 0.015, 0.015]
        ...
        # PARAMS FOR THE GRASP FLOOR PRIMITIVE:
        self.grasp_xy_rotate_floor = -0.002748661208897829 - 0.04
        ...
        if env.simulation:
            self.grasp_xy_rotate_ground = (
                0.0022661592811346054 - 0.04 + 0.0012392520904541023
            )  # -0.007273907661437987#+ 0.0012835693359374983#+ 0.00017907857894897475
            self.grasp_h_rotate_floor = (
                -0.0036394810304045677 + 0.006026389598846434
            )  # 0.009729766845703126#0.0013717865943908697#+ 0.006139404773712158
        else:
            self.grasp_xy_rotate_ground = (
                0.0022661592811346054 - 0.02 - 0.004453001022338867
            )  # 3+ 0.008769088983535766#+ 0.015980666875839232
            self.grasp_h_rotate_floor = 0.0  # -0.0036394810304045677 + 0.011232354640960694#-0.00455500602722168
        ...
        if env.simulation:
            self.gain_xy_final_rotate_ground = (
                556.7359089851379  # 803.606367111206#400.0#562.4382019042969
            )
            ...
            self.force_factor_ground_rot_rotate_ground = (
                0.2  # 0.14088733196258546#0.2#0.12187141180038452
            )
            ...
            self.force_factor_rotate_ground = (
                0.6  # 0.42808679342269895#0.38862146139144893#0.3449445694684983#0.61
            )
        ...
        # PARAMS ROTATE LIFT
        if env.simulation:
            self.grasp_xy_rotate_lift = (
                0.0022661592811346054 - 0.04 + 0.00303934335708618
            )  # -0.02
            self.grasp_h_rotate_lift = (
                0.0 - 0.017098468542099
            )  # -0.00025067448616027804
        else:
            self.grasp_xy_rotate_lift = (
                0.0022661592811346054 - 0.04 - 0.00622488021850586
            )  # + 0.009622406959533692
            self.grasp_h_rotate_lift = (
                0.0 + 0.014247066974639896
            )  # + 0.005761625766754149
        ...
        if env.simulation:
            ...
            self.pos_gain_impedance_rotate_lift = (
                0.02185524344444275  # 0.01539862297475338
            )
            ...
            self.pos_gain_impedance_rotate_lift = (
                0.026721017956733706  # 0.02095555767416954
            )
        ...
        if env.simulation:
            ...
            self.orient_grasp_xy_lift = (
                -0.002748661208897829
                - 0.04
                + 0.0034238076210021985
                + 0.019999999999999997
            )  # -0.02229524075984955
            self.orient_grasp_h_lift = (
                0.0 + 0.02 - 0.02838870149105787
            )  # -0.012001985907554625
        else:
            self.orient_grasp_xy_lift = (
                -0.002748661208897829 - 0.04 + 0.012545742988586423
            )
        ...
        if env.simulation:
            ...
            self.orient_gain_xy_lift_lift = (
                700  # 842.1571612358093#672.3513275384903#829.543000459671
            )
            self.orient_gain_z_lift_lift = (
                750  # 1378.0078768730164#1425.8808374404907#1101.177817583084
            )
            ...
            self.orient_force_factor_lift = (
                0.59  # 0.35#0.5970059156417846#0.4772495344281197#0.5929181218147278
            )
            ...
            self.orient_force_factor_rot_lift = (
                0.0017775391042232514  # 0.0011627759784460067
            )
