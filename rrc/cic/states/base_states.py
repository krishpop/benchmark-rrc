...
import pybullet
from rrc.cic import rotation_primitives as rotation_primitives_cic
from rrc.cic.math_tools import rpy2Mat
from rrc.cic.utils import get_robot_and_obj_state
from rrc.env.cube_env import ActionType
from rrc.mp.states import State
...
class IdlePrimitive(State):
    ...
    def __call__(self, obs, info={}):
        ...
        action = self.get_action(
            position=np.clip(
                self.reference,
                self.action_space_limits.low,
                self.action_space_limits.high,
            ),
...
class IdlePrimitiveChangeGoal(IdlePrimitive):
    ...
    def is_finished(self, iteration):
        if iteration > self.iter_primitive_started + self.duration:
            self.env.set_goal(
                orientation=self.env.goal_list[-1][0], pos=self.env.goal_list[-1][1]
            )
...
class InitResetPrimitive(State):
    ...
    def __init__(self, env, kinematics, params):
        ...
        self.duration = 150  # duration of the primitive in timesteps
        self.DEBUG = False
        self.iter_primitive_started = -1
        self.iteration = -1
        self.reference = None
    ...
    def __call__(self, obs, info={}):
        if self.iteration == -1:
            ...
            offset = 15
            self.unit_vec_x = np.matmul(
                np.asarray(rpy2Mat(0, 0, np.deg2rad(0 + offset))).reshape(3, 3),
                unit_vec,
            )
            self.unit_vec_y = np.matmul(
                np.asarray(rpy2Mat(0, 0, np.deg2rad(240 + offset))).reshape(3, 3),
                unit_vec,
            )
            self.unit_vec_z = np.matmul(
                np.asarray(rpy2Mat(0, 0, np.deg2rad(120 + offset))).reshape(3, 3),
                unit_vec,
            )

            self.reference = self.kinematics.inverse_kinematics_3_fingers(
                obs["robot_position"], self.unit_vec_x, self.unit_vec_y, self.unit_vec_z
            )
...
class ApproachObjectViapoints(State):
    ...
    def __call__(self, obs, info={}):
        if self.iteration == -1:
            ...
            pos1, pos2, pos3 = self.grasp.get_finger_position(
                robot_state,
                state,
            ...
            self.reference = self.kinematics.inverse_kinematics_3_fingers(
                robot_state[0], pos1, pos2, pos3
            )

        robot_state, state = get_robot_and_obj_state(obs, self.kinematics)
        reference = robot_state[0] + np.clip(
            self.reference - robot_state[0],
        ...
        action = self.get_action(
            position=np.clip(
                reference, self.action_space_limits.low, self.action_space_limits.high
            ),
            frameskip=self._frameskip,
        )
        self.iteration += 1
...
class MoveCubeFloorPrimitive(State):
    ...
    def __call__(self, obs, info={}):
        ...
        robot_state, state = get_robot_and_obj_state(
            obs, self.kinematics, last_states=self.last_states
        )
        ...
        torque = self.kinematics.imp_ctrl_3_fingers(
            [robot_state[0], robot_state[0], robot_state[0]],
            [0.0 * dir1, 0.0 * dir2, 0.0 * dir3],
            [4, 8, 12],
        ...
        add_torque = self.kinematics.add_additional_force_3_fingers(
            robot_state[0],
            self.force_factor,
        ...
        action = self.get_action(
            torque=np.clip(
                torque, self.action_space_limits.low, self.action_space_limits.high
            ),
            frameskip=self._frameskip,
        )
        self.iteration += 1
...
class MoveCubeFloorPrimitiveLvl1(MoveCubeFloorPrimitive):
    ...
    def __init__(self, env, kinematics, params, object, grasp, stop_early):
        super(MoveCubeFloorPrimitiveLvl1, self).__init__(
            env, kinematics, params, object, grasp, stop_early
        )
        self.duration = 300
    ...
    def is_finished(self, robot_state, cube_state, iteration):
        ...
        if iteration > self.iter_primitive_started + self.duration:
            ...
            dist_law = (
                np.sum(dir_progress) >= 0.0 and np.sqrt(np.sum(curr_dist ** 2)) > 2.5
            )
            if dist_law:
                self.reset()
                return True
...
class MoveCubeFloorPrimitiveLvl4(MoveCubeFloorPrimitive):
    ...
    def __init__(
        ...
        super(MoveCubeFloorPrimitiveLvl4, self).__init__(
            env,
            kinematics,
            params,
            object,
...
class MoveLiftCubePrimitive(State):
    ...
    def update_gain(self):
        ...
        #     return
        if (
            self.iteration % self.interval == 0
            and self.interval_ctr < self.max_interval_ctr
        ):
    ...
    def __call__(self, obs, info={}):
        ...
        if (
            not (self.start_lift)
            and (self.iteration > self.iter_primitive_started + self.init_dur)
            and (xy_dist < self.switch_mode)
        ):
        ...
        if self.start_lift:
            target[2] = state[0][2] + np.clip(
                target[2] - state[0][2], -self.clip_height, self.clip_height
            )
        else:
            target[2] = 0.0325
...
class MoveLiftCubePrimitiveLvl2(MoveLiftCubePrimitive):
    ...
    def __init__(self, env, kinematics, params, object, grasp, stop_early):
        super(MoveLiftCubePrimitiveLvl2, self).__init__(
            env, kinematics, params, object, grasp, stop_early
        )
        self.duration = 1000
    ...
    def is_finished(self, robot_state, cube_state, iteration):
        ...
        if self.start_lift:
            target[2] = cube_state[0][2] + np.clip(
                target[2] - cube_state[0][2], -self.clip_height, self.clip_height
            )
        else:
            target[2] = 0.0325
...
class MoveLiftCubePrimitiveLvl4(MoveLiftCubePrimitive):
    ...
    def __init__(self, env, kinematics, params, object, grasp, stop_early):
        super(MoveLiftCubePrimitiveLvl4, self).__init__(
            env, kinematics, params, object, grasp, stop_early
        )
    ...
    def is_finished(self, robot_state, cube_state, iteration):
        ...
        if iteration > self.iter_primitive_started + self.duration:
            ...
            dir_progress = (self.last_states[:, 0] - self.last_states[:, 50]) * 100
            dist_law = (
                np.sum(dir_progress) >= 0.0 and np.sqrt(np.sum(curr_dist ** 2)) > 4.5
            )
            if dist_law:
                self.reset()
                return True
...
class MoveLiftCubeOrientPrimitive(State):
    ...
    def __init__(self, env, kinematics, params, object, grasp, stop_early):
        ...
        self.pos_gain_impedance_ground = (
            self.params.orient_pos_gain_impedance_ground_lift
        )
        ...
        self.planner = rotation_primitives_cic.RotationPrimitives(
    ...
    def __call__(self, obs, info={}):
        ...
        if self.start_moving:
            self.planner.set_goal(
                rotation_primitives_cic.to_H(
                    np.asarray(
                        pybullet.getMatrixFromQuaternion(obs["goal_object_orientation"])
                    ).reshape(3, 3)
                )
            )
            self.planner.set_current_pose(
                rotation_primitives_cic.to_H(
                    np.asarray(pybullet.getMatrixFromQuaternion(state[1])).reshape(3, 3)
                )
            )
            ...
            if not (self.start_lift):
                if self.force_factor_rot_ground != 0.0:
                    ...
                    dir_rot[np.argmax(np.abs(dir_rot))] = (
                        np.sign(magnitude)
                        * np.sign(dir_rot[np.argmax(np.abs(dir_rot))])
                        * 1.0
                    )
                    ...
                    rotation_dir = np.matmul(
                        np.asarray(pybullet.getMatrixFromQuaternion(state[1])).reshape(
                            3, 3
                        ),
                        dir_rot,
                    )
                    dir_rotatione = (
                        rotation_value * rotation_dir * self.force_factor_rot_ground
                    )

                    (
                        add_torque_force,
                        add_torque_torque,
                    ) = self.kinematics.comp_combined_torque(
                        copy.deepcopy(robot_state[0]),
                        [0, 0, 0],
                        dir_rotatione,
                        [1, 1, 1],
                        state[0],
                ...
                if self.force_factor_rot_lift != 0.0:
                    ...
                    #               (reference_vector / np.linalg.norm(reference_vector)))
                    gain = np.cos(magnitude)  # this should be easier here
                    self.integral_value_orient += (
                        self.params.orient_int_orient_gain
                        * np.clip(gain, 0, 1)
                        * np.sign(magnitude)
                    )
                    self.integral_value_orient = np.clip(
                        self.integral_value_orient, -10, 10
                    )
                    rotation_value = (
                        np.pi
                        / 2
                        * (
                            np.clip((1 - gain) * 2.5, 0, 1)
                            + np.abs(self.integral_value_orient)
                        )
                    )
                    rotation_dir = np.matmul(
                        np.asarray(pybullet.getMatrixFromQuaternion(state[1])).reshape(
                    ...
                    dir_rotatione = (
                        rotation_value * rotation_dir * self.force_factor_rot_lift
                    )
...
class MoveLiftCubeOrientPrimitiveLvl4(MoveLiftCubeOrientPrimitive):
    ...
    def __init__(self, env, kinematics, params, object, grasp, stop_early):
        super(MoveLiftCubeOrientPrimitiveLvl4, self).__init__(
            env, kinematics, params, object, grasp, stop_early
        )
        self.duration = 1000
    ...
    def is_finished(self, robot_state, cube_state, iteration):
        ...
        if iteration > self.iter_primitive_started + self.duration:
            ...
            dir_progress = (self.last_states[:, 0] - self.last_states[:, 50]) * 100
            dist_law = (
                np.sum(dir_progress) >= 0.0 and np.sqrt(np.sum(curr_dist ** 2)) > 6.0
            )
            if dist_law:
                self.reset()
                return True
...
class RotateCubeFloorPrimitive(State):
    ...
    def __call__(self, obs, info={}):
        ...
        if self.iteration == 0:
            ...
            self.reference_vector, _, _ = self.grasp.get_edge_directions(
                robot_state, state
            )
            # print (self.reference_vector)
            self.reference_vector = np.matmul(
                rot_mat.as_matrix(), self.reference_vector
            )
            # print (self.reference_vector)
            # input ("WAIT")
        ...
        if self.additional_force:
            ...
            gain = np.dot(
                (curr_reference_vector / np.linalg.norm(curr_reference_vector)),
                (self.reference_vector / np.linalg.norm(self.reference_vector)),
            )
            # TODO: check this gain scheduling
            rotation_value = (
                np.pi / 2 * np.clip((1 - gain) * self.params.gain_scheduling, 0, 1)
            )
            dir_rotatione = (
                rotation_value
                * np.sign(self.dir_rot_magnitude)
                * self.dir_rot
                / np.sqrt(np.sum((self.dir_rot) ** 2))
            )
            ...
            add_torque_force, add_torque_torque = self.kinematics.comp_combined_torque(
                copy.deepcopy(robot_state[0]),
                [0, 0, 0],
...
class RotateCubeFloorPrimitiveLvl4(RotateCubeFloorPrimitive):
    ...
    def __init__(
        ...
        super(RotateCubeFloorPrimitiveLvl4, self).__init__(
            env,
            kinematics,
            params,
            object,
...
class RotateCubeLiftPrimitive(State):
    ...
    def __call__(self, obs, info={}):
        ...
        # potentially add additional component
        # TODO: to find out: on the floor might be even more stable without this additional component,...
        add_torque = self.kinematics.add_additional_force_3_fingers_center_gain(
            robot_state[0],
            self.force_factor,
            self.force_factor,
            self.grasp.get_edge_directions(robot_state, state),
        ...
        if self.additional_force:
            ...
            dir_rotatione = (
                1
                * self.dir_rot[idx_rotation_axes]
                * dir_rotatione
                / np.sqrt(np.sum((dir_rotatione) ** 2))
            )
...
class RotateCubeLiftPrimitiveLvl4(RotateCubeLiftPrimitive):
    ...
    def __init__(
        ...
        super(RotateCubeLiftPrimitiveLvl4, self).__init__(
            env,
            kinematics,
            params,
            object,
...
class ApproachObjectViapointsSpecialGrasp(ApproachObjectViapoints):
    ...
    def __init__(
        ...
        super(ApproachObjectViapointsSpecialGrasp, self).__init__(
            env,
            kinematics,
            params,
            object,
        ...
        if self.iteration == -1:
            ...
            if self.assign_fingers:
                ...
                if vertical_idx < idx_rotation_axes:
                    indicator -= 1
                if indicator == 0:
                    self.grasp.assign_fingers_y(
                        robot_state, state, direction=np.sign(np.sum(self.dir))
                    )
                else:
                    self.grasp.assign_fingers_x(
                        robot_state, state, direction=np.sign(np.sum(self.dir))
                    )
...
from rrc.mp.grasping import get_planned_grasp
from rrc.mp.grasping.grasp_sampling import GraspSampler
...
class SetGraspPrimitive(State):
    ...
    def __call__(self, obs, info={}):
        ...
        if state_finished:
            ...
            custom_grasp = [grasp_sampler.get_custom_grasp(obs["robot_tip_positions"])]
            try:
                grasp, path = get_planned_grasp(
                    self.env,
                    obs["object_position"],
