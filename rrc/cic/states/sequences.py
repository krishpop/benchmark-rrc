from rrc.mp.align_rotation import roll_and_pitch_aligned, get_yaw_diff
from rrc.mp.states import State
from rrc.cic.states.base_states import *

from rrc.cic import states as states_cic
...
class BringCenterPrimitive(State):
    ...
    def __init__(self, env, kinematics, parameters, object, grasp):
        self.goinitpose = states_cic.InitResetPrimitive(
            env, kinematics=kinematics, params=parameters
        )
        self.approach = states_cic.ApproachObjectViapoints(
            ...
            object,
            grasp,
            parameters.approach_grasp_xy,
            parameters.approach_grasp_h,
            parameters.approach_duration,
            parameters.approach_clip_pos,
            stop_early=True,
            assign_fingers=True,
        )
        self.bring_center = states_cic.MoveCubeFloorPrimitiveLvl4(
            env,
            kinematics,
            parameters,
            object,
        ...
        self.reset_state = states_cic.ApproachObjectViapoints(
            ...
            object,
            grasp,
            parameters.reset_grasp_xy,
            parameters.reset_grasp_h,
            parameters.reset_duration,
            parameters.reset_clip_pos,
            stop_early=True,
            assign_fingers=False,
        )

    def connect(self, next_state, done_state):
        self.goinitpose.connect(next_state=self.approach, done_state=done_state)
        self.approach.connect(next_state=self.bring_center, done_state=done_state)
        self.bring_center.connect(next_state=self.reset_state, done_state=done_state)
        self.reset_state.connect(next_state=next_state, done_state=done_state)
...
class BringFinalPoseOrientationPrimitive(State):
    ...
    def __init__(self, env, kinematics, parameters, object, grasp, grasp1=None):
        ...
        if grasp1 is None:
            ...
            self.set_cic_grasp = states_cic.SetGraspFromOutsidePrimitive(
                env, kinematics, parameters, object, grasp1
            )
            grasp_to_be_used = grasp1

        self.move_cube_lift = states_cic.MoveLiftCubeOrientPrimitiveLvl4(
            env, kinematics, parameters, object, grasp_to_be_used, False
        )
        self.reset_state = states_cic.ApproachObjectViapoints(
            env,
            kinematics,
    ...
    def connect(self, next_state, done_state):
        ...
        if self.grasp1 is None:
            self.approach.connect(next_state=self.move_cube_lift, done_state=done_state)
        else:
            self.approach.connect(next_state=self.set_cic_grasp, done_state=done_state)
            self.set_cic_grasp.connect(
                next_state=self.move_cube_lift, done_state=done_state
            )

        self.move_cube_lift.connect(next_state=self.reset_state, done_state=done_state)
        self.reset_state.connect(next_state=next_state, done_state=done_state)
...
class BringFinalPosePrimitive(State):
    ...
    def __init__(self, env, kinematics, parameters, object, grasp):
        ...
        self.move_cube_lift = states_cic.MoveLiftCubePrimitiveLvl2(
            env, kinematics, parameters, object, grasp, False
        )
        self.reset_state = states_cic.ApproachObjectViapoints(
            env,
            kinematics,
...
class BringFinalPosePrimitiveLvl4(BringFinalPosePrimitive):
    ...
    def __init__(self, env, kinematics, parameters, object, grasp):
        super(BringFinalPosePrimitiveLvl4, self).__init__(
            env, kinematics, parameters, object, grasp
        )
...
class CorrectOrientationGroundPrimitive(State):
    ...
    def __init__(self, env, kinematics, parameters, object, grasp, grasp1=None):
        ...
        self.rotate = states_cic.RotateCubeFloorPrimitiveLvl4(
            env,
            kinematics,
            parameters,
            object,
    ...
    def connect(self, next_state, done_state):
        self.goinitpose.connect(next_state=self.approach, done_state=done_state)
        if self.grasp1 is None:
            self.approach.connect(next_state=self.rotate, done_state=done_state)
        else:
            self.approach.connect(next_state=self.set_cic_grasp, done_state=done_state)
            self.set_cic_grasp.connect(next_state=self.rotate, done_state=done_state)

        self.rotate.connect(next_state=self.reset_state, done_state=done_state)
        self.reset_state.connect(next_state=next_state, done_state=done_state)
...
class CorrectOrientationAirPrimitive(State):
    ...
    def __init__(self, env, kinematics, parameters, object, grasp, grasp1=None):
        ...
        self.approachrotlift = states_cic.ApproachObjectViapointsSpecialGrasp(
            env,
            kinematics,
            parameters,
            object,
        ...
        self.rotate_lift = states_cic.RotateCubeLiftPrimitiveLvl4(
            env,
            kinematics,
            parameters,
            object,
    ...
    def connect(self, next_state, done_state):
        self.goinitpose.connect(next_state=self.approachrotlift, done_state=done_state)
        if self.grasp1 is None:
            self.approachrotlift.connect(
                next_state=self.rotate_lift, done_state=done_state
            )
        else:
            self.approachrotlift.connect(
                next_state=self.set_cic_grasp, done_state=done_state
            )
            self.set_cic_grasp.connect(
                next_state=self.rotate_lift, done_state=done_state
            )

        self.rotate_lift.connect(next_state=self.reset_state, done_state=done_state)
        self.reset_state.connect(next_state=next_state, done_state=done_state)
...
class AlignAxesPrimitive(State):
    ...
    def __init__(self, env, kinematics, parameters, object, grasp, grasp1=None):
        ...
        self.bring_center = BringCenterPrimitive(
            env, kinematics, parameters, object, grasp
        )
        self.rotate_ground = CorrectOrientationGroundPrimitive(
            env, kinematics, parameters, object, grasp, grasp1=grasp1
        )
        self.rotate_air = CorrectOrientationAirPrimitive(
            env, kinematics, parameters, object, grasp, grasp1=grasp1
        )
    ...
    def connect(self, next_state, done_state):
        self.bring_center.connect(next_state=self, done_state=done_state)
        self.rotate_ground.connect(next_state=self, done_state=done_state)
        self.rotate_air.connect(next_state=self, done_state=done_state)
        ...
        if np.abs(np.rad2deg(sequence[0][1])) > 10:
            ...
            if xy_dist > 0.05:
                ...
                if not (vertical_idx == np.argmax(np.abs(dir_rot))):
                    self.rotate_air.set_rotation_directions(
                        dir_rot, [state[0][0], state[0][1], 0.0325]
                    )
                    ...
                    self.rotate_ground.set_rotation_directions(
                        dir_rot, [state[0][0], state[0][1], 0.0325]
                    )

        elif np.abs(np.rad2deg(sequence_direct[0][1]) > 45):
            sequence_direct[0][1] = np.clip(
                sequence_direct[0][1], -np.deg2rad(60), np.deg2rad(60)
            )
            print("rotate minor")
            desired = [0, 0, 0.0325]
            xy_dist = np.sqrt(np.sum((desired[:2] - state[0][:2]) ** 2))
            if xy_dist > 0.05:
                ...
                dir_rot[np.argmax(np.abs(dir_rot))] = np.sign(
