...
from rrc.cic import grasping_class as cic_grasping_class
...
class CICStateMachineLvl1(states_cic.StateMachineCIC):
    def build(self):
        self.init = states_cic.IdlePrimitive(
            self.env, kinematics=self.main_ctrl.kinematics, params=self.parameters
        )
...
class CICStateMachineLvl2(states_cic.StateMachineCIC):
    def build(self):
        ...
        self.set_cic_grasp1.connect(
            next_state=self.move_cube_lift, done_state=self.done
        )
...
class CICwithPG(states_cic.StateMachineCIC):
    def build(self):
        ...
        self.wait = states.WaitState(self.env, 30 if self.env.simulation else 1000)
        self.failure = states.FailureState(self.env)

        # define transitions between states
        self.goto_init_pose.connect(
            next_state=self.align_object, failure_state=self.failure
        )
        self.align_object.connect(
            next_state=self.planned_grasp, failure_state=self.failure
        )
        self.planned_grasp.connect(
            next_state=self.set_cic_grasp, failure_state=self.align_object
        )

        self.set_cic_grasp.connect(
            next_state=self.move_cube_lift, done_state=self.failure
        )
