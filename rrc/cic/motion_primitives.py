...
class PrimitiveObject:
    ...
    def is_finished(
        self, robot_state, cube_state, cube_state_filtered, goal, iteration
    ):
        pass
...
class IdlePrimitive(PrimitiveObject):
    ...
    def step(self, robot_state, state, cube_state_filtered, goal, iteration, initial):
        ...
        thisdict = {"torque": np.zeros((9)), "position": self.reference}
        return thisdict
    ...
    def step(self, robot_state, state, cube_state_filtered, goal, iteration, initial):
        if initial:
            ...
            self.reference = self.kinematics.inverse_kinematics_3_fingers(
                robot_state[0], self.unit_vec_x, self.unit_vec_y, self.unit_vec_z
            )
...
class MoveObjectFloorPrimitive(PrimitiveObject):
    ...
    def step(self, robot_state, state, cube_state_filtered, goal, iteration, initial):
        ...
        thisdict = {"torque": torque, "position": np.zeros((9))}
        return thisdict
...
class MoveCubeFloorPrimitiveLvl1(MoveCubeFloorPrimitive):
    ...
    def __init__(self, kinematics, params, object, grasp, stop_early):
        super(MoveCubeFloorPrimitiveLvl1, self).__init__(
            kinematics, params, object, grasp, stop_early
        )
        self.duration = 300
...
class MoveLiftCubePrimitive(PrimitiveObject):
    ...
    def step(self, robot_state, state, cube_state_filtered, goal, iteration, initial):
        ...
        if (
            not (self.start_lift)
            and (iteration > self.iter_primitive_started + self.init_dur)
