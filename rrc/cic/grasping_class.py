...
class BasicGraspingPrimitive:
    def __init__(self, kinematics, params, object):
        ...
        self.error = 9999.0
        self.error_matrix = np.ones((3, 3)) * 999.0
        self.d_error = 9999.0
    ...
    def get_grasping_viapoints(self, cube_state, robot_state, center=True):
        ...
        pos_f_w = self.object.in_world(
            cube_state=cube_state, pos_c=pos_f_c, offset_prop=True
        )
        pos_f_candidates = [
            pos_f_w[:, :3],
            pos_f_w[:, 3:6],
        ...
        for candidate in pos_f_candidates:
            check = np.sum(candidate[2, :] < 0.0)
            if check < 1:
                candidates_clean.append(candidate)

        if len(candidates_clean) > 0:
            self.goal_target, _, __ = self.combinatorial_matching(
                fingers_poses=robot_xyz, cube_targets=pos_f_candidates
            )
    ...
    def is_satisfied(self, cube_state, des_cube_state, robot_state):
        robot_state_np = np.stack(
            (robot_state[2][0], robot_state[2][1], robot_state[2][2]), 1
        )
    ...
    def assign_y_internal(self, edge_arr, p0, p1, p2, direction):
        ...
        pot_ret = [
            [max_dist, add_grasp1, add_grasp2],
            [add_grasp2, max_dist, add_grasp1],
            [add_grasp1, add_grasp2, max_dist],
        ]
        cent = [[1, 0, 0], [0, 1, 0], [0, 0, 1]]
        config1 = np.sum(
            (edge_arr[:, max_dist] - p0) ** 2
            + (edge_arr[:, add_grasp1] - p1) ** 2
            + (edge_arr[:, add_grasp2] - p2) ** 2
        )
        config2 = np.sum(
            (edge_arr[:, max_dist] - p1) ** 2
            + (edge_arr[:, add_grasp1] - p2) ** 2
            + (edge_arr[:, add_grasp2] - p0) ** 2
        )
        config3 = np.sum(
            (edge_arr[:, max_dist] - p2) ** 2
            + (edge_arr[:, add_grasp1] - p0) ** 2
        ...
        moment_dir = np.dot(
            self.skew(self.unit_vector(edge_arr[:, 0] - edge_arr[:, 2])),
            self.unit_vector(edge_arr[:, 1] - edge_arr[:, 3]),
        )
    ...
    def assign_x_internal(self, edge_arr, p0, p1, p2, direction):
        ...
        moment_dir = np.dot(
            self.skew(self.unit_vector(edge_arr[:, 1] - edge_arr[:, 3])),
            self.unit_vector(edge_arr[:, 0] - edge_arr[:, 2]),
        )
        if direction == -1 or direction == 1:
            if np.sign(moment_dir[2]) * np.sign(direction) == 1:
                return pot_ret_1, cent_1, dist_1
...
class TwoFingerGrasp(BasicGraspingPrimitive):
    ...
    def assign_fingers(self, robot_state, cube_state, off_x=0.0, off_y=0.0, off_z=0.0):
        ...
        (
            ...
            vertical_axes_idx,
            axes_dir,
        ) = self.object.compute_edge_loc_entire_cube(cube_state, off_x, off_y, off_z)
        self.asign_arr, self.cent_arr = self.assign_y(
            locations, robot_state[2][0], robot_state[2][1], robot_state[2][2]
        )

    def get_finger_position(
        self, robot_state, cube_state, off_x=0.0, off_y=0.0, off_z=0.0, use_z=True
    ):
        ...
        return (
            locations[:, self.asign_arr[0]],
            locations[:, self.asign_arr[1]],
            locations[:, self.asign_arr[2]],
        )

    def get_edge_directions(self, robot_state, cube_state):
        edge_dir = self.object.compute_edge_dir_entire_cube(cube_state)
        return (
            edge_dir[:, self.asign_arr[0]],
            edge_dir[:, self.asign_arr[1]],
            edge_dir[:, self.asign_arr[2]],
        )
...
class ThreeFingerGrasp(BasicGraspingPrimitive):
    ...
    def assign_fingers(self, robot_state, cube_state, off_x=0.0, off_y=0.0, off_z=0.0):
        ...
        (
            ...
            vertical_axes_idx,
            axes_dir,
        ) = self.object.compute_edge_loc_entire_cube(
            cube_state, off_x, off_y, off_z, compute_vertical=True
        )
        self.asign_arr, self.cent_arr = self.assign_target(
            locations,
            cube_state[3],
            robot_state[2][0],
            robot_state[2][1],
    ...
    def assign_fingers_unrestricted(
        self, robot_state, cube_state, off_x=0.0, off_y=0.0, off_z=0.0
    ):
        off_y += self.excl_add_offset
        """
        This function should assign the fingers to where you want to grasp potnetially,...
        ...
        self.asign_arr, self.cent_arr = self.assign_target_unrestricted(
            locations,
            cube_state[3],
            robot_state[2][0],
            robot_state[2][1],
    ...
    def assign_fingers_x(
        self, robot_state, cube_state, direction=0, off_x=0.0, off_y=0.0, off_z=0.0
    ):
        off_y += self.excl_add_offset
        """
        ...
        self.asign_arr, self.cent_arr = self.assign_x(
            locations,
            robot_state[2][0],
            robot_state[2][1],
            robot_state[2][2],
    ...
    def get_axis(self, cube_state):
        ...
        (
            ...
            vertical_axes_idx,
            axes_dir,
        ) = self.object.compute_edge_loc_entire_cube(cube_state)
...
class ThreeFingerGraspExternal(ThreeFingerGrasp):
    ...
    def get_finger_position(
        self, robot_state, cube_state, off_x=0.0, off_y=0.0, off_z=0.0, use_z=False
    ):
        """
        This function should return the desired positions of the fingers,...
        """
        ...
        # Step 1: calculate the locations:
        pos1, pos2, pos3 = self.object.compute_edge_loc_special(
            cube_state, off_x, off_y, off_z
        )
        # Step 2: use the assignment and return the finger positions
