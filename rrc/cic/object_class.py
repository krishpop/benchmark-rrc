...
class Object:
    ...
    def faces_xyz(self, cube_state):
        ...
        rot = np.asarray(
            rpy2Mat(cube_state[2][0], cube_state[2][1], cube_state[2][2])
        ).reshape(3, 3)

        face = np.eye(3) * np.array(
            [self._CUBE_WIDTH_x, self._CUBE_WIDTH_y, self._CUBE_WIDTH_z]
        )
        faces_in_cube = np.concatenate((face, -face), 1)
        faces_in_world = np.matmul(rot, faces_in_cube) + np.tile(
            cube_state[0][:, None], [1, 6]
        )
    ...
    def offseted_faces_xyz(self, cube_state, offset):
        ...
        face = np.eye(3) * np.array(
            [
                self._CUBE_WIDTH_x + offset,
                self._CUBE_WIDTH_y + offset,
                self._CUBE_WIDTH_z + offset,
            ]
        )
        faces_in_cube = np.concatenate((face, -face), 1)
        faces_in_world = np.matmul(faces_in_cube.T, rot).T + np.tile(
            cube_state[0][:, None], [1, 6]
        )
    ...
    def in_world(self, cube_state, pos_c, offset_prop=False):
        ...
        pos_w = np.matmul(pos_c.T, rot_c2w).T + np.tile(
            cube_state[0][:, None], [1, pos_c.shape[1]]
        )
        return pos_w
...
class Cuboid(Object):
    ...
    def compute_edge_loc_entire_cube(
        self, cube_state, off_x=0.0, off_y=0.0, off_z=0.0, compute_vertical=False
    ):
...
class Cube(Object):
    ...
    def compute_edge_loc_entire_cube(
        ...
        rot_p0 = np.asarray(pybullet.getMatrixFromQuaternion(cube_state[1])).reshape(
            3, 3
        )
    ...
    def set_directions(self, robot_state, cube_state):
        ...
        self.edge_dir1[np.argmax(np.abs(self.edge_dir1))] = np.sign(
            self.edge_dir1[np.argmax(np.abs(self.edge_dir1))]
        )
        ...
        self.edge_dir2[np.argmax(np.abs(self.edge_dir2))] = np.sign(
            self.edge_dir2[np.argmax(np.abs(self.edge_dir2))]
        )
        ...
        self.edge_dir3[np.argmax(np.abs(self.edge_dir3))] = np.sign(
            self.edge_dir3[np.argmax(np.abs(self.edge_dir3))]
        )
