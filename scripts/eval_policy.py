import wandb
import gym
from rrc.env import make_env, cube_env
...
def run_hog_env(env, hog_env):
    obs = env.reset()
    init, goal = obs['achieved_goal'], obs['desired_goal']
    initializer = initializers.FixedInitializer(env.difficulty, init, goal)
    hog_env.unwrapped.initializer = initializer
